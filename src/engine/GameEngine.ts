export type Move = 'rock' | 'paper' | 'scissors';
export type GameMode = 'pvc' | 'pvp';
export type Result = 'win' | 'lose' | 'tie';

export const MOVE_ICONS: Record<Move, string> = {
  rock: '🪨',
  paper: '📄',
  scissors: '✂️',
};

export const MOVE_NAMES: Record<Move, string> = {
  rock: 'Rock',
  paper: 'Paper',
  scissors: 'Scissors',
};

const RULES: Record<Move, Move[]> = {
  rock: ['scissors'],
  paper: ['rock'],
  scissors: ['paper'],
};

export const ALL_MOVES: Move[] = ['rock', 'paper', 'scissors'];

export function getWinner(move1: Move, move2: Move): Result {
  if (move1 === move2) return 'tie';
  return RULES[move1]?.includes(move2) ? 'win' : 'lose';
}

export function getRandomMove(): Move {
  return ALL_MOVES[Math.floor(Math.random() * ALL_MOVES.length)];
}

export function getCounterMove(move: Move): Move {
  const counters: Record<Move, Move> = {
    rock: 'paper',
    paper: 'scissors',
    scissors: 'rock',
  };
  return counters[move];
}